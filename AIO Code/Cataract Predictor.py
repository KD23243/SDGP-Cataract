...
def registerFile():
    def createAccount():
        ...
        if name.get() == "":
            tkinter.messagebox.showinfo("No username", "No username")
        else:
    ...
    name = StringVar()
    nameEntry = Entry(register, textvariable=name, relief='flat', bg='#a1bce7').place(x=437, y=142, height=40,
                                                                                      width=200)

    email = StringVar()
    emailEntry = Entry(register, textvariable=email, relief='flat', bg='#a1bce7').place(x=437, y=203, height=40,
                                                                                        width=200)

    password = StringVar()
    passwordEntry = Entry(register, textvariable=password, relief='flat', show='*', bg='#a1bce7').place(x=437, y=263,
                                                                                                        height=40,
                                                                                                        width=200)

    signup = ImageTk.PhotoImage(Image.open("imageData/signup.png"))
    signupbtn = Button(register, image=signup, command=createAccount, borderwidth=0, bd=0, highlightthickness=0).place(
        x=425, y=328)

    haveAccount = ImageTk.PhotoImage(Image.open("imageData/haveAccount.png"))
    haveAccountbtn = Button(register, image=haveAccount, command=gotosignin, borderwidth=0, bd=0,
                            highlightthickness=0).place(x=402, y=404)
...
def loginFile():
    ...
    username = StringVar()
    usernameEntry = Entry(login, textvariable=username, relief='flat', bg='#a1bce7').place(x=440, y=180, height=40,
                                                                                           width=200)

    password = StringVar()
    passwordEntry = Entry(login, textvariable=password, relief='flat', show='*', bg='#a1bce7').place(x=440, y=245,
                                                                                                     height=40,
                                                                                                     width=200)
    ...
    signinbtn1 = Button(login, image=signinbtn, command=validateLogin, borderwidth=0, bd=0, highlightthickness=0).place(
        x=433, y=318)

    donthaveAccount = ImageTk.PhotoImage(Image.open("imageData/donthaveAccount.png"))
    donthaveAccountbtn = Button(login, image=donthaveAccount, command=gotoregister, borderwidth=0, bd=0,
                                highlightthickness=0).place(x=410, y=398)
